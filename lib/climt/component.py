...
class Component:
    ...
    def compute(self, ForcedCompute=False):
        ...
        else:                            Output = dict( list(zip(self.FromExtension, OutputValues )) )
    ...
    def report(self):
        print('CliMT component:    %s' % self.Name)
        keys = list(self.State.keys())
        print('State variables:')
        print('Prognostic:')
        for key in keys:
            if key in self.Prognostic:
                print('  %s: %s (%s)' % (key,
                                          self.State.KnownFields[key][0],
                                          self.State.KnownFields[key][1]))
        print('Diagnostic:')
        for key in keys:
            if key in self.Diagnostic:
                 print('  %s: %s (%s)' % (key,
                                          self.State.KnownFields[key][0],
                                          self.State.KnownFields[key][1]))
        print('Fixed:')
        for key in keys:
            if key not in self.Prognostic and \
               key not in self.Diagnostic and \
               key in (self.ToExtension+self.FromExtension):
                print('  %s: %s (%s)' % (key,
                                          self.State.KnownFields[key][0],
                                          self.State.KnownFields[key][1]))
    ...
    def _checkUnused(self,kwargs):
        ...
        '''
        unused = []
        io_keys = ['RestartFile','OutputFile','OutputFreq','OutputFields','PrintOutputProgress','ElapsedTime']
    ...
    def _getAxisLength(self, AxisName, **kwargs):
        ...
        if n is None:
            n = eval('get_n%s()' % AxisName)

        # Check if extension enforces axis dimension, ensure consistency
        try:
            n_ext = eval('self.Extension.get_n%s()' % AxisName)
        except:
            n_ext = n
        assert n_ext == n, \
