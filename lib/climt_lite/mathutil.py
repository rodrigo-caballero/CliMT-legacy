...
find a root x of f using Ridder's method. Parameter fnvals=(f(x1),f(x2)) is optional.
	...
	if f1==0:
		...
		return x2
	elif (f1>=0. and f2>=0. and f1>f2) or (f1<0. and f2<0. and f1<f2):
		raise BracketingException('initial interval does not bracket a root: root probably to the right of interval')
	elif (f1>=0. and f2>=0. and f1<f2) or (f1<0. and f2<0. and f1>f2):
		raise BracketingException('initial interval does not bracket a root: root probably to the left of interval')
	x4 = 123456789.
	for j in range(max_iterations):
		x3 = (x1+x2)/2
		f3 = f(x3)
