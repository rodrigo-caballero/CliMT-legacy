c
c $Id: pagrid.h,v 1.1.1.1 2004/02/27 05:44:07 rca Exp $
c $Author: rca $
c
C
...
C
      ...
      integer pflds     ! Number of fields in master field list
...
C
      parameter(pbflnb=(7 + 2*pcnst)*plndlv + (5+pcnst)*plond)
C
...
C
      ...
      parameter(ptapes = 6)
C
C 8 fields in master list are pcnst-dependent 2 fields occur only if pcnst > 1.
C Extra "1" is for sea ice thickness (COUP_SOM).  25 LSM fields.
C 90 CCM3 single + multi-level fields.  plevmx sub-surface flds
C
      parameter(pflds=90+8*pcnst+2*(pcnst-1)+plevmx+25+1)
C
